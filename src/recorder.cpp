...
void Recorder::startWriting() {
    ...
    const auto status = mcap_.open(write_filename_, mcap_options_);
    if (!status.ok()) {
...
bool Recorder::checkSize()
{
    ...
    {
        if (mcap_.getOutput().size() > options_.max_size)
        {
            if (options_.split)
            {
                stopWriting();
...
void Recorder::doRecordSnapshotter() {
    ...
    while (nh.ok() || !queue_queue_.empty()) {
        ...
        const auto status = mcap_.open(write_filename, mcap_options_);
        if (!status.ok()) {
            ROS_ERROR("Error writing: %s", status.message.c_str());
            return;
        }
